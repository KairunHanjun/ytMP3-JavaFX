...
import com.iseng.binarytree.youtube.DownloadProgressCallback;
...
public class StreamProcessExtractorCancelable extends Thread {
    ...
    public StreamProcessExtractorCancelable(int iterator,StringBuffer buffer, InputStream stream, DownloadProgressCallback callback) {
        this.stream = stream;
        this.buffer = buffer;
        this.callback = callback;
        this.setName("StreamProcessExtractorCancelable ("+iterator+")");
        this.start();
    }
    ...
    public void run() {
        try {
            ...
            int nextChar;
            while ((nextChar = stream.read()) != -1 && !isInterrupted()) {
